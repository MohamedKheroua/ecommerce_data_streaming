...
async def post_invoice_item(item: InvoiceItem): #body awaits a json with invoice item information
    ...
    try:
        # Evaluate the timestamp and parse it to datetime object we can work with
        date = datetime.strptime(item.InvoiceDate, "%d/%m/%Y %H:%M")

        print("Timestamp found: ", date)

        # Replace stange date with new datetime
    ...
    # Will be thrown by datetime if date does not fit
